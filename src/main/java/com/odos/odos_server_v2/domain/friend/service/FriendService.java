...
public class FriendService {
    ...
    public void sendFriendRequest(Long toMemberId) {
        ...
        // 기존 신청 확인 (대기 중)
        if (friendRequestRepository.existsByFromMemberAndToMemberAndStatus(fromMember, toMember, FriendRequestStatus.PENDING)) {
            throw new CustomException(ErrorCode.FRIEND_REQUEST_ALREADY_EXISTS);
        }

        // 거절된 신청이 있으면 상태를 PENDING으로 변경
        friendRequestRepository.findByFromMemberAndToMember(fromMember, toMember)
                .ifPresent(request -> {
    ...
    public List<FriendRequestResponseDto> getReceivedFriendRequests() {
        Member currentMember = getCurrentMember();
        List<FriendRequest> requests = friendRequestRepository.findByToMemberAndStatus(currentMember, FriendRequestStatus.PENDING);
    ...
    public void deleteFriend(Long friendMemberId) {
        Member currentMember = getCurrentMember();
        Member friendMember = memberRepository.findById(friendMemberId)
                .orElseThrow(() -> new CustomException(ErrorCode.MEMBER_NOT_FOUND));

        // 친구 관계 존재 확인
        Friend friend = friendRepository.findByMemberAndFriendMember(currentMember, friendMember)
                .orElseThrow(() -> new CustomException(ErrorCode.FRIEND_NOT_EXISTS));

        friendRepository.delete(friend);

        // 상대방의 친구 목록에서도 삭제 (양방향)
        friendRepository.findByMemberAndFriendMember(friendMember, currentMember)
                .ifPresent(friendRepository::delete);

        // 해당하는 친구 신청이 있으면 삭제
        friendRequestRepository.findByFromMemberAndToMember(currentMember, friendMember)
                .ifPresent(friendRequestRepository::delete);
        friendRequestRepository.findByFromMemberAndToMember(friendMember, currentMember)
                .ifPresent(friendRequestRepository::delete);
    }
    ...
    public void blockMember(Long blockedMemberId) {
        ...
        // 친구 관계가 있으면 해제
        friendRepository.findByMemberAndFriendMember(currentMember, blockedMember)
                .ifPresent(friendRepository::delete);
        friendRepository.findByMemberAndFriendMember(blockedMember, currentMember)
                .ifPresent(friendRepository::delete);

        // 해당하는 친구 신청이 있으면 삭제
        friendRequestRepository.findByFromMemberAndToMember(currentMember, blockedMember)
                .ifPresent(friendRequestRepository::delete);
        friendRequestRepository.findByFromMemberAndToMember(blockedMember, currentMember)
                .ifPresent(friendRequestRepository::delete);
    }
    ...
    public void unblockMember(Long blockedMemberId) {
        ...
        com.odos.odos_server_v2.domain.friend.entity.BlockList blockList =
                blockListRepository.findByMemberAndBlockedMember(currentMember, blockedMember)
    ...
    public List<FriendResponseDto> getBlockList() {
        ...
        return blockList.stream()
                .map(block -> FriendResponseDto.builder()
    ...
    public MemberRelationResponseDto getMemberRelation(Long targetMemberId) {
        ...
        // 내가 보낸 친구 신청 확인
        if (friendRequestRepository.existsByFromMemberAndToMemberAndStatus(currentMember, targetMember, FriendRequestStatus.PENDING)) {
            return MemberRelationResponseDto.builder()
                    .memberId(targetMemberId)
        ...
        // 내가 받은 친구 신청 확인
        if (friendRequestRepository.existsByFromMemberAndToMemberAndStatus(targetMember, currentMember, FriendRequestStatus.PENDING)) {
            return MemberRelationResponseDto.builder()
                    .memberId(targetMemberId)
