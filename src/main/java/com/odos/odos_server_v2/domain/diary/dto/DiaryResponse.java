...
public class DiaryResponse {
  ...
  public static DiaryResponse from(Member member, Diary diary) {
    List<Long> achievements = diary.getDiaryGoals().stream().map(DiaryGoal::getId).toList();
    ...
    Boolean isLiked =
        diary.getLikes().stream().anyMatch(like -> like.getMember().getId().equals(member.getId()));
